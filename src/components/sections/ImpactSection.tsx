          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 lg:gap-10 border-y border-outline-variant/20 dark:border-outline-variant/10 py-10 lg:py-14">
            {stats.map((stat) => (
              <div key={stat.label} className="flex flex-col gap-1.5">
                <span className="font-(family-name:--font-sora) text-3xl sm:text-4xl lg:text-[48px] font-bold text-primary leading-tight">
                  {stat.prefix}
                  <CountUp end={stat.value} suffix={stat.suffix} />
                </span>
                <p className="font-(family-name:--font-dm-sans) text-[10px] sm:text-xs font-medium text-text-secondary uppercase tracking-widest">
